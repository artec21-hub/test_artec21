...
function get_member_login($mb_id, $pt_id)
{
	if($pt_id == "admin"){
		$login_check = sql_fetch("select * from shop_member where id = TRIM('$mb_id') and grade <> 2");
	}else{
		$login_check = sql_fetch("select * from shop_member where id = TRIM('$mb_id') and grade = 2");
	}
	return $login_check;
...
// 포인트 부여
function insert_point($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0, $od_no='', $gubun='')
{
    global $config;
...
function is_soldout($gs_id)
{
	...
	if($row['cnt']) {
		$sql = " select io_id, io_type, io_stock_qty,king_pcode 
		...
		for($i=0; $row=sql_fetch_array($result); $i++) {
			...
			$stock_qty = get_option_stock_qty($row['king_pcode']);
...
*/
function get_option_stock_qty($king_pcode)
{
	$stock = king_post_stock($king_pcode);//curl 수행 결과, json 형식으로 받아 온다.
//	$stock = 1;
...
// 물품구매 추천인 포인트 적립
function choochun_save($od_no,$paymethod,$mb_id,$pt_id,$use_price)
{
	if($paymethod == "무통장" || $paymethod == "예치금"){
		
		$b_point = 0;

		$b_point = ($use_price * 0.04);

		// 본인 구매 4% 포인트 부여
		insert_point($mb_id, $b_point, '주문번호'.$od_no.'로 현금 구매 4% 포인트 적립', '@member', $mb_id, '본인현금구매포인트',0,$od_no,'판매포인트');	


	}

	$sql = " select grade from shop_member where id = '{$pt_id}' ";
	$row = sql_fetch($sql);

	$c_point = 0;
	

	if($row['grade']=="4" || $row['grade']=="7"){
		$c_point = ($use_price * 0.03);
	}elseif($row['grade']=="5" || $row['grade']=="8"){
		$c_point = ($use_price * 0.02);
	}elseif($row['grade']=="6" || $row['grade']=="9"){
		$c_point = ($use_price * 0.01);
	}

	// 추천인 구매 포인트 부여
	insert_point($pt_id, $c_point, $mb_id.'의 추천인 주문번호'.$od_no.'로 인한 적립', '@member', $pt_id, $mb_id.' 추천인 구매적립',0,$od_no,'추천포인트');

}

function choochun_cancel($od_no,$paymethod,$mb_id,$pt_id,$use_price)
{
	if($paymethod == "무통장"){
		...
		// 본인 구매 4% 포인트 차감
		insert_point($mb_id, -$b_point, $od_no.' 주문취소로 현금 구매 4% 포인트 적립취소', '@member', $mb_id, '본인현금구매포인트취소',0,$od_no,'판매포인트');	
	...
	// 추천인 구매 포인트 부여
	insert_point($pt_id, -$c_point, $mb_id.'의 추천인 주문취소 '.$od_no.'로 인한 적립취소', '@member', $pt_id, $mb_id.' 추천인 구매적립취소',0,$od_no,'추천포인트');
...
// 예치금 충전 처리
function cash_charge($id,$to_id,$cash,$action,$gubun,$plusminus)
{

	$sql1 = " select cash as cash from shop_member where id = '{$id}' ";
	$row1 = sql_fetch($sql1);
	...
	$cash_remain = (int)$row1['cash'] + (int)$cash;

	$sql = " insert shop_cash_history
					set id = '{$id}',
					to_id = '{$to_id}',
					action = '{$action}',
					gubun = '{$gubun}',
					...
					action_date = '".TB_TIME_YMDHIS."'";
	sql_query($sql);

	$sql = " update shop_member set cash = cash + {$cash} where id = '{$id}' ";
	sql_query($sql);

}

// '구매확정' 예치금 계산
function yechigrm_calc($od_no)
{
	...
	while($ct=sql_fetch_array($res)) {
		...
		if($ct['shop_id'] == "admin"){
			cash_charge("admin",$ct['gong_id'],(int)$ct['supply_price']+(int)$ct['baesong_price'],$ct['od_no']." 주문건 구매확정으로 예치금 차감","도매","minus");
		}else{
			cash_charge("admin",$ct['gong_id'],(int)$ct['supply_price']+(int)$ct['baesong_price'],$ct['od_no']." 주문건 구매확정으로 예치금 차감","도매","minus");
			cash_charge($ct['shop_id'],"admin",(int)$ct['somae_supply_price']+(int)$ct['baesong_price'],$ct['od_no']." 주문건 구매확정으로 예치금 차감","가맹점","minus");
		}
	}
	
}
// '구매확정'취소 예치금 계산
function yechigrm_calc_cancel($od_no)
{
	...
	//	if($ct['shop_id'] == "admin"){
	//		cash_charge("admin",$ct['gong_id'],(int)$ct['supply_price']+(int)$ct['baesong_price'],$ct['od_no']." 주문건 구매취소로 예치금 증감","도매","plus");
	//	}else{
			cash_charge("admin",$ct['gong_id'],(int)$ct['supply_price']+(int)$ct['baesong_price'],$ct['od_no']." 주문건 구매취소로 예치금 증감","도매","plus");
			cash_charge($ct['mb_id'],"admin",(int)$ct['somae_supply_price']+(int)$ct['baesong_price'],$ct['od_no']." 주문건 구매취소로 예치금 증감","가맹점","plus");
	//	}
	}
...
function get_sale_price($gs_id)
{
	...
	$gs = sql_fetch("select goods_price,use_aff,lv1_price,lv2_price,lv3_price,lv4_price,lv5_price,lv6_price,lv7_price,lv8_price,lv9_price from shop_goods where index_no = '$gs_id'");

	$lv1_price = $gs['lv1_price'];
	$lv2_price = $gs['lv2_price'];
	$lv3_price = $gs['lv3_price'];
	$lv4_price = $gs['lv4_price'];
	$lv5_price = $gs['lv5_price'];
	$lv6_price = $gs['lv6_price'];
	$lv7_price = $gs['lv7_price'];
	$lv8_price = $gs['lv8_price'];
	$lv9_price = $gs['lv9_price'];
...
// 상품 소매공급가격정보의 배열을 리턴
function get_somae_price($gs_id,$pt_id)
{
	//global $member;

	$gb = sql_fetch("select grade from shop_member where id = '{$pt_id}'");
	$gs = sql_fetch("select goods_price,use_aff,lv1_price,lv2_price,lv3_price,lv4_price,lv5_price,lv6_price,lv7_price,lv8_price,lv9_price from shop_goods where index_no = '$gs_id'");
