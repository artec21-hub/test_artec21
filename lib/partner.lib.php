...
function insert_cash_charge($mb_id,$cash)
{

	$sql1 = " select cash as cash from shop_member where id = '{$mb_id}' ";
	$row1 = sql_fetch($sql1);

	$cash_money = (int)$cash + 330000;
	$cash_remain = (int)$row1['cash'] + (int)$cash;

	$sql = " insert shop_cash_history
					set id = '{$mb_id}',
					to_id = 'admin',
					action = '가맹점가입 예치금 {$cash} 적립합니다.',
					gubun = '가맹점',
					plus_minus = '+',
					cash_now = {$cash},
	...
	$sql = " insert shop_cash_charge
					set id = '{$mb_id}',
					to_id = 'admin',
					action = '가맹점가입 가입금+예치금 {$cash_money} 적립합니다.',
					gubun = '가맹점',
					charge_money = {$cash_money},
					result = 'Y',
					input_date = '".TB_TIME_YMDHIS."'";
	sql_query($sql);

	$sql = " update shop_member set cash = cash + {$cash} where id = '{$mb_id}' ";
	sql_query($sql);
