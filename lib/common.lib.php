...
function tree_category($catecode)
{
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$sql_cnt = " select count(*) as cnt from shop_goods where isopen=1 and (ca_id like '{$row['catecode']}%' or ca_id2 like '{$row['catecode']}%' or ca_id3 like '{$row['catecode']}%') ";
		...
		$str .= "<li style=\"width:{$li_width}%\"{$addclass}><a href=\"{$href}\">{$row['catename']}({$g_count})</a></li>".PHP_EOL;
	}
...
// 5차카테고리
function tree_category2($catecode)
{
	//echo $catecode;
	global $pt_id;
	...
	$mod = 5; // 1줄당 노출 수
	$li_width = (float)(94 / $mod);
	$sql = "SELECT b.catecode,b.catename,COUNT(*) as g_cnt FROM shop_goods AS a,shop_category AS b WHERE ((a.ca_id LIKE '$t_catecode%') OR (a.ca_id2 LIKE '$t_catecode%') OR (a.ca_id3 LIKE '$t_catecode%')) AND ((a.ca_id LIKE '003%') OR (a.ca_id2 LIKE '003%') OR (a.ca_id3 LIKE '003%')) AND b.catecode LIKE '003%' AND (a.ca_id = b.catecode OR a.ca_id2 = b.catecode OR a.ca_id3 = b.catecode) GROUP BY b.upcate ORDER BY b.upcate";

	//$sql = "select * {$sql_common} {$sql_where} and upcate = '$catecode' {$sql_order} ";
	//echo $sql;
	$result = sql_query($sql);
	for($i=0; $row=sql_fetch_array($result); $i++) {
		if($i==0) $str .= '<ul class="sub_tree2">'.PHP_EOL;
	...
	for($i=0; $row=sql_fetch_array($result); $i++)
	{
		...
		$str .= "<li>\n";
			...
			$str .= "<dl>\n";
				...
				$str .= "<dd class=\"pname\">{$it_explan} / {$it_model}<br>{$it_name}</dd>\n";
...
function get_listtype_best2($type, $width, $height, $rows, $li_css='')
{
	global $pt_id;	
	...
	for($i=0; $row=sql_fetch_array($result); $i++)
	{
		...
		if($row['normal_price'] > $it_amount && !$is_uncase) {
			...
			$str .= "<li>\n";
				...
				$str .= "<dd>\n";
						$str .= "<div>\n";
						$str .= "<p class=\"pname\">{$it_explan} / {$it_model}<br>{$it_name}</p>\n";
...
function get_order_list($row, $amount, $baesong_search='')
{
	...
	if(!$row['shop_id'] || $row['shop_id'] == 'admin')
		$disp_pt_id = '본사';
	else {
		$mb = get_member($row['shop_id'], 'name');
		$mb_name = get_sideview($row['shop_id'], $mb['name']);
		if(!$mb_name) $mb_name = '정보없음';
		$disp_pt_id = $mb_name.'<span class="list_mb_id">('.$row['shop_id'].')</span>';
	}
...
function get_price($gs_id, $msg='<span>원</span>')
{
	...
	if(is_soldout($gs['index_no'])) {
		...
		if($gs['price_msg']) {
			...
			$str = "<span class=\"memopen\">회원전용몰</span>";
...
//  상품 상세페이지 구매하기, 장바구니, 찜 버튼
function get_list_buy_button($msg, $t_idx)
{
	global $gs, $pt_id;
	...
	for($i=1; $i<=2; $i++) {
		switch($i){
			case '1':
				$sw_css = " bdark twhite w80 h30 lh30 abtn bda1 m20";
				$sw_name = "구매하기";
				$sw_direct = "buy";
				break;
			case '2':
				$sw_css = " bbgray twhite w80 h30 lh30 abtn bda1 m20";
				$sw_name = "장바구니";
				$sw_direct = "cart";
				break;
		...
		if($msg) {
			...
			if($sw_direct == "wish") {
				...
				$str .= "<a href=\"javascript:fbuyform_submit('".$sw_direct."',".$t_idx.");\"><input type=\"button\" value=".$sw_name." class=\"".$sw_css."\"></a>";
			}
		}
	}
...
function get_item_options($gs_id, $subject)
{
	...
	if($subj_count > 1) {
		...
		for($i=0; $row=sql_fetch_array($result); $i++) {
			...
			$stock = king_post_stock($row['king_pcode']);
			...
			$select .= '<option value="'.$row['io_id'].','.$row['io_price'].','.$stock.','.$amt.','.$row['king_pcode'].'">'.$row['io_id'].$price.$soldout.'</option>'.PHP_EOL;
		}
		$select .= '</select>'.PHP_EOL;
...
// 상품 선택옵션
function get_list_item_options($gs_id, $subject, $rows)
{
	if(!$gs_id || !$subject)
		return '';
	...
	if($subj_count > 1) { //옵션갯수 2개이상
		...
		for($i=0; $i<$subj_count; $i++) {
			...
			if($opt_count) {
				...
				if($i > 0)
					$disabled = ' disabled="disabled"';
				$str .= '<dl>'.PHP_EOL;
				$str .= '<dt><label for="it_option_'.$seq.'_'.$rows.'">'.$subj[$i].'</label></dt>'.PHP_EOL;

				$select  = '<select id="it_option_'.$seq.'_'.$rows.'" class="list_it_option wfull"'.$disabled.'>'.PHP_EOL;
				$select .= '<option value="">(필수) 선택하세요</option>'.PHP_EOL;
				for($k=0; $k<$opt_count; $k++) {
					$opt_val = $opt[$k];
					if($opt_val) {
		...
		$str .= '<dl>'.PHP_EOL;
		$str .= '<dt><label for="it_option_1_'.$rows.'">'.$subj[0].'</label></dt>'.PHP_EOL;

		$select  = '<select id="it_option_1_'.$rows.'" class="list_it_option wfull">'.PHP_EOL;
		$select .= '<option value="">(필수) 선택하세요</option>'.PHP_EOL;
		for($i=0; $row=sql_fetch_array($result); $i++) {
			if($row['io_price'] >= 0)
				$price = '&nbsp;&nbsp;(+'.display_price($row['io_price']).')';
			...
			$select .= '<option value="'.$row['io_id'].','.$row['io_price'].','.$stock.','.$amt.','.$rows.','.$row['king_pcode'].'">'.$row['io_id'].$price.$soldout.'</option>'.PHP_EOL;
		}
		$select .= '</select>'.PHP_EOL;
