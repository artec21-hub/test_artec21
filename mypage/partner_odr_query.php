...
if(isset($taxbill_check))		 $qstr .= "&taxbill_check=$taxbill_check";
...
if($code == 'partner_odr_list'){ // 주문리스트(전체)
	$where[] = " dan != 0 ";
}elseif($code == 'partner_odr_taxbill'){
	$where[] = " taxbill_yes = 'Y' ";
}elseif($code == 'partner_odr_taxsave'){
	$where[] = " taxsave_yes = 'Y' ";
}else{
...
if($taxbill_check)
	$where[] = " taxbill_check = '$taxbill_check' ";

if($taxsave_check)
	$where[] = " taxsave_check = '$taxsave_check' ";
