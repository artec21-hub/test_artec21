...
$sql = " select gb_no,gb_sale from shop_member_grade";
$result = sql_query($sql);
for($i=0; $row=sql_fetch_array($result); $i++) {
	if($row['gb_no'] == 2){
		$goods_sale = $row['gb_sale'];
		$lv2_sale = $row['gb_sale'];
	}elseif($row['gb_no'] == 1){
		$lv1_sale = $row['gb_sale'];
	}elseif($row['gb_no'] == 3){
		$lv3_sale = $row['gb_sale'];
	}elseif($row['gb_no'] == 4){
		$lv4_sale = $row['gb_sale'];
	}elseif($row['gb_no'] == 5){
		$lv5_sale = $row['gb_sale'];
	}elseif($row['gb_no'] == 6){
		$lv6_sale = $row['gb_sale'];
	}elseif($row['gb_no'] == 7){
		$lv7_sale = $row['gb_sale'];
	}elseif($row['gb_no'] == 8){
		$lv8_sale = $row['gb_sale'];
	}elseif($row['gb_no'] == 9){
		$lv9_sale = $row['gb_sale'];
	}
}

$supply_price = conv_number($_POST['supply_price']); //공급가격
/*
$lv1_price = round($supply_price + ($supply_price * ($lv1_sale/100)),-1);
$lv2_price = round($supply_price + ($supply_price * ($lv2_sale/100)),-1);
$lv3_price = round($supply_price + ($supply_price * ($lv3_sale/100)),-1);
$lv4_price = round($supply_price + ($supply_price * ($lv4_sale/100)),-1);
$lv5_price = round($supply_price + ($supply_price * ($lv5_sale/100)),-1);
$lv6_price = round($supply_price + ($supply_price * ($lv6_sale/100)),-1);
$lv7_price = round($supply_price + ($supply_price * ($lv7_sale/100)),-1);
$lv8_price = round($supply_price + ($supply_price * ($lv8_sale/100)),-1);
$lv9_price = round($supply_price + ($supply_price * ($lv9_sale/100)),-1);
$goods_price = round($supply_price + ($supply_price * ($goods_sale/100)),-1);
*/
$lv1_price = round($supply_price / (1-($lv1_sale/100)),-1);
$lv2_price = round($supply_price / (1-($lv2_sale/100)),-1);
$lv3_price = round($supply_price / (1-($lv3_sale/100)),-1);
$lv4_price = round($supply_price / (1-($lv4_sale/100)),-1);
$lv5_price = round($supply_price / (1-($lv5_sale/100)),-1);
$lv6_price = round($supply_price / (1-($lv6_sale/100)),-1);
$lv7_price = round($supply_price / (1-($lv7_sale/100)),-1);
$lv8_price = round($supply_price / (1-($lv8_sale/100)),-1);
$lv9_price = round($supply_price / (1-($lv9_sale/100)),-1);
$goods_price = round($supply_price / (1-($goods_sale/100)),-1);
...
$value['gong_id']		= get_session('ss_mb_id'); //공급자아이디
...
$value['lv1_price']	= $lv1_price;
$value['lv2_price']	= $lv2_price;
$value['lv3_price']	= $lv3_price;
$value['lv4_price']	= $lv4_price;
$value['lv5_price']	= $lv5_price;
$value['lv6_price']	= $lv6_price;
$value['lv7_price']	= $lv7_price;
$value['lv8_price']	= $lv8_price;
$value['lv9_price']	= $lv9_price;
