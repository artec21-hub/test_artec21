...
<div class="tbl_frm01">
	...
	<tr>
		...
		<td>
			<?php echo radio_checked('taxbill_check', $taxbill_check,  '', '전체'); ?>
			<?php echo radio_checked('taxbill_check', $taxbill_check, 'Y', '발급완료'); ?>
			<?php echo radio_checked('taxbill_check', $taxbill_check, 'N', '미발급'); ?>
		</td>
	</tr>
	</tbody>
	</table>
...
<form name="forderlist" id="forderlist" method="post" action="./partner_odr_update.php" onsubmit="return forderlist_submit(this);" >
...
<div class="tbl_head01">
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if($row['taxbill_check'] == 'N')
			$taxbill_final = '<span class="fc_107">미발급</span>';
		else
			$taxbill_final = '<span class="fc_00f">발급완료</span>';
	...
	<tr class="<?php echo $bg; ?>">
		...
		<td rowspan="<?php echo $rowspan; ?>"><?php echo $row['company_name']; ?></td>
		<td rowspan="<?php echo $rowspan; ?>"><?php echo $row['company_addr']; ?></td>
		<td rowspan="<?php echo $rowspan; ?>"><?php echo $row['company_owner']; ?></td>
		<td rowspan="<?php echo $rowspan; ?>"><?php echo $row['company_saupja_no']; ?></td>
		<td rowspan="<?php echo $rowspan; ?>"><?php echo $row['company_item']; ?></td>
		<td rowspan="<?php echo $rowspan; ?>"><?php echo $row['company_service']; ?></td>

		<td rowspan="<?php echo $rowspan; ?>"><?php echo $taxbill_final; ?></td>
...
<div class="tbl_frm01">
	...
	<tbody>
	<tr>
		<th scope="row">선택한 세금계산서를</th>
		<td>
			<input type="submit" name="act_button" value="세금계산서발급완료" class="btn_medium red" onclick="document.pressed=this.value">
			<!--input type="submit" name="act_button" value="세금계산서발급취소" class="btn_medium white" onclick="document.pressed=this.value"-->
		</td>
	</tr>
	</tbody>
	</table>
...
function forderlist_submit(f)
{
    ...
    if(!confirm("선택하신 세금계산서의 상태를 '"+document.pressed+"'상태로 변경하시겠습니까?"))
        return false;
