...
$sql_common = " from shop_cash_charge ";
$sql_search = " where to_id = '$mb_id' and gubun = '도매' ";
$sql_order  = " order by ch_id desc ";


if($sf2) {
    $sql_search .= " and result = '$sf2' ";
}

if($sf3) {
    $sql_search .= " and tax_result = '$sf3' ";
}
	...
	<tr class="<?php echo $bg; ?>">
		...
		<td><?php 
			if($row['tax_result'] == "Y"){
				...
				echo "<a href='seller_cash_charge_update.php?mode=tax&ch_id={$row['ch_id']}'><p class='btn_small gray marl10'>미처리</p></a>";
			}
			?></td>
		...
		<td><?php 
			if($row['result'] == "Y"){
				...
				echo "<a href='seller_cash_charge_update.php?cash={$row['charge_money']}&action={$row['action']}&to_id={$row['to_id']}&id={$row['id']}&plusminus=plus&mode=charge&ch_id={$row['ch_id']}'><p class='btn_small gray marl10'>미처리</p></a>";
			}
			?></td>
