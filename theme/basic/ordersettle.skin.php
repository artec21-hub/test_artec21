...
require_once(TB_SHOP_PATH.'/settle/orderform.1.php');
?>
...
require_once(TB_SHOP_PATH.'/settle/orderform.2.php');
?>
...
function forderform_check(f)
{
	...
	if(f.method.value == '계좌이체') {
		pay('bank')
	}

	if(f.method.value == '가상계좌') {
		pay('vbank')
	}

    if(f.method.value == '신용카드') {
		pay('card');
    }
...
{
    var tot_price = parseInt(f.plainTrdAmt.value);
