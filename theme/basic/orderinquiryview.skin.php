...
<div id="con_lf">
	...
	<section id="sod_fin_pay">
		...
		<div class="tbl_frm01 tbl_wrap">
				...
				<td>
					...
					if($od['paymethod'] == '휴대폰')
					{
						if($od['od_pg'] == 'lg') {
							...
							$hp_receipt_script = 'window.open(\'https://pg.nicepay.co.kr/issue/IssueLoader.jsp?TID='.$od['od_tno'].'&type=0\',\'popupIssue\',\'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes, resizable=yes,width=420,height=540\');';
						}
					?>
			...
			// 현금영수증 발급을 사용하는 경우에만
			if($default['de_taxsave_use'] && $od['od_pg'] != 'nicepay') {
			...
			<tr>
				...
				if($od['od_cash'])
				{
					if($od['od_pg'] == 'lg') {
						...
						$cash = unserialize($od['od_cash_info']);
						$cash_receipt_script = 'window.open(\'https://pg.nicepay.co.kr/issue/IssueLoader.jsp?TID='.$cash['TID'].'&type=1\',\'popupIssue\',\'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=414,height=622\');';
					} else if($od['od_pg'] == 'settle') {
						$cash = unserialize($od['od_cash_info']);
						$cash_receipt_script = 'window.open(\'https://pg.nicepay.co.kr/issue/IssueLoader.jsp?TID='.$cash['TID'].'&type=1\',\'popupIssue\',\'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=414,height=622\');';
					}
	...
	<section id="sod_fin_cancel">
		...
		<div id="sod_fin_cancelfrm">
			...
			<input type="hidden" name="paymethod"  value="<?php echo $od['paymethod']; ?>">
