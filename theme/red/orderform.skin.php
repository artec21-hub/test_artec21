...
<div class="tbl_head02 tbl_wrap">
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$sql = " select SUM(IF(io_type = 1, (io_price * ct_qty), ((io_price + ct_price) * ct_qty))) as price,
						...
						SUM(IF(io_type = 1, (io_supply_price * ct_qty), ((io_supply_price + ct_somae_supply_price) * ct_qty))) as somae_supply_price,
		...
		$somae_supply_price = $sum['somae_supply_price'];
	...
	<tr>
		<td class="tac">
			...
			<input type="hidden" name="gong_id[<?php echo $i; ?>]" value="<?php echo $gs['gong_id']; ?>">
			<input type="hidden" name="supply_price[<?php echo $i; ?>]" value="<?php echo $supply_price; ?>">
			<input type="hidden" name="somae_supply_price[<?php echo $i; ?>]" value="<?php echo $somae_supply_price; ?>">
...
<input type="hidden" name="mb_cash" value="<?php echo $member['cash']; ?>">
	...
	<div class="tbl_frm01 tbl_wrap">
		...
		<tr>
			...
			<td class="td_label">
				...
				if($member['grade'] == "4" || $member['grade'] == "5" || $member['grade'] == "6") {
					//if($is_member && ($tot_price <= $member['cash'])) {
						echo '<input type="radio" name="paymethod" id="paymethod_cash" value="예치금" onclick="calculate_paymethod(this.value);"> <label for="paymethod_cash">예치금결제</label>'.PHP_EOL;
					//}
				}
		...
		<tr id="cash_tr" style="display:none;">
			<th scope="row">예치금결제</th>
			<td>
				<input type="text" name="use_cash" value="0" class="frm_input" size="12" onkeyup="calculate_temp_cash(this.value);this.value=number_format(this.value);" style="font-weight:bold;"> 원 보유예치금 : <?php echo display_point($member['cash']); ?>				
			</td>
		</tr>
		<?php } ?>
		<?php
...
function calculate_order_price() {
	...
	//var mb_cash   = parseInt($("input[name=use_cash]").val().replace(/[^0-9]/g, "")); //예치금결제
...
function fbuyform_submit(f) {
	...
	var mb_cash		= parseInt(f.mb_cash.value);
	...
	if(getRadioVal(f.paymethod) == '예치금') {

		var temp_cash	= parseInt(no_comma(f.use_cash.value));	

		if(f.use_cash.value == '') {
			alert('예치금사용 금액을 입력하세요. 사용을 원치 않을경우 0을 입력하세요.');
			f.use_cash.value = 0;
			f.use_cash.focus();
			return false;
		}
		...
		if(temp_cash > 0 && temp_cash > mb_cash) {
			alert('예치금사용 금액은 현재 보유예치금 보다 클수 없습니다.');
			f.tot_price.value = number_format(String(tot_price));
			f.use_cash.value = 0;
			f.use_cash.focus();
			return false;
		...
		if(temp_cash > 0 && temp_cash > tot_price) {
			alert('예치금사용 금액은 최종결제금액 보다 클수 없습니다.');
			f.tot_price.value = number_format(String(tot_price));
			f.use_cash.value = 0;
			f.use_cash.focus();
			return false;
	...
	f.use_cash.value = no_comma(f.use_cash.value);
...
function calculate_temp_point(val) {
	...
	if(val == '' || !checkNum(no_comma(val))) {
		...
		f.use_cash.value =  number_format(String(tot_price - temp_point));
		f.tot_price.value = number_format(String(tot_price - temp_point));
	}
}

function calculate_temp_cash(val) {
	var f = document.buyform;
	var temp_cash = parseInt(no_comma(f.use_cash.value));
	var sell_price = parseInt(f.org_price.value);
	var send_cost2 = parseInt(f.baesong_price2.value);
	...
	if(val == '' || !checkNum(no_comma(val))) {
		alert('dPclrma 사용액은 숫자이어야 합니다.');
		f.tot_price.value = number_format(String(tot_price));
		f.use_cash.value = 0;
		f.use_cash.focus();
		return;
	} else {
		f.tot_price.value = number_format(String(tot_price - temp_cash));
	}
}
...
function calculate_paymethod(type) {
	...
	var mb_cash   = parseInt($("input[name=mb_cash]").val()); // 보유예치금
	var tot_price  = sell_price + send_cost2 - mb_coupon;
	...
	switch(type) {
		case '무통장':
			...
			$("input[name=use_point]").val(0);
			$("input[name=use_point]").attr("readonly", false);
			$("input[name=use_cash").val(0);
			$("input[name=use_cash]").attr("readonly", true);
			calculate_order_price();
			<?php if(!$config['company_type']) { ?>
			$("#tax_section").show();
			<?php } ?>
		...
		case '포인트':
			...
			$("input[name=use_cash").val(0);
			$("input[name=use_cash]").attr("readonly", false);
			calculate_order_price();
			<?php if(!$config['company_type']) { ?>
			$("#tax_section").hide();
			$(".taxbill_fld").hide();
		...
		case '예치금':
			$("#cash_tr").show();
			$("#bank_section").hide();
			$("input[name=use_cash]").val(number_format(String(tot_price)));
			$("input[name=use_cash]").attr("readonly", true);
			$("input[name=use_point]").val(0);
			$("input[name=use_point]").attr("readonly", false);
			calculate_order_price();
