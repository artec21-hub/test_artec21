		...
		<div id="hd">
			...
			<div id="hd_inner">
				...
				<div class="hd_category">					
					<img class="gijong" src="/img/gijong_w.png" width="130px" height="35px"><img class="jepum" src="/img/jepum_b.png" width="130px" height="35px"><img class="brand" src="/img/brand_b.png" width="130px" height="35px">
				</div>
			</div>
			<div id="gnb">
				<div id="gnb_inner">
								...
								<ul class="second-level-menu">
									...
									$res2 = sql_query_cgy($row['catecode']);
										while($row2 = sql_fetch_array($res2)) {
											$href2 = TB_SHOP_URL.'/list.php?ca_id='.$row2['catecode'];
									?>
									<li><a href="<?php echo $href2; ?>"><?php echo $row2['catename']; ?></a>
									<?php
										$r3 = sql_query_cgy($row2['catecode'], 'COUNT');
										if($r3['cnt'] > 0) {
									?>
										<ul class="third-level-menu">
											<?php
												...
												while($row3 = sql_fetch_array($res3)) {
													$href3 = TB_SHOP_URL.'/list.php?ca_id='.$row3['catecode'];
											?>
											<li><a href="<?php echo $href3; ?>"><?php echo $row3['catename']; ?></a></li>
											<?php } ?>										
										</ul>
									</li>
									<?php } ?>
					...
					<?php
					if($default['de_skin1_jepum_menu']) {
						$list_gnb = unserialize(base64_decode($default['de_skin1_jepum_menu']));
					...
					<script>
						...
						$(function(){							
							$('.hd_category .gijong').mouseover(function(){
								$(".gnb_li2").hide();
								$(".gnb_li").show();
								$(".gijong").attr("src", $(".gijong").attr("src").replace("_b","_w"));
								$(".jepum").attr("src", $(".jepum").attr("src").replace("_w","_b"));
								$(".brand").attr("src", $(".brand").attr("src").replace("_w","_b"));
							});

							$('.hd_category .jepum').mouseover(function(){
								$(".gnb_li").hide();
								$(".gnb_li2").show();
								$(".gijong").attr("src", $(".gijong").attr("src").replace("_w","_b"));
								$(".jepum").attr("src", $(".jepum").attr("src").replace("_b","_w"));
								$(".brand").attr("src", $(".brand").attr("src").replace("_w","_b"));
							});

							$('.hd_category .brand').click(function(){								
								$(".gnb_li").hide();
								$(".gnb_li2").hide();
								$(".gijong").attr("src", $(".gijong").attr("src").replace("_w","_b"));
								$(".jepum").attr("src", $(".jepum").attr("src").replace("_w","_b"));
								$(".brand").attr("src", $(".brand").attr("src").replace("_b","_w"));
								$(location).attr('href',"/shop/brand.php");
							});
						});
