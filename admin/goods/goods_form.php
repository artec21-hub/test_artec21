		...
		<td>
			...
			$(function() {
				...
				$("#supply_price").keyup(function(){
					var price = $("#supply_price").val().replace(/,/g, '');
					if( price != ''){
						$.ajax({
							type: 'POST',
							url: '/admin/goods/goods_price_ajax.php',
							data: { supply_price : price },
							dataType: 'json',
							success: function(result){
								var level4_price = String(result.level4_price).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
								var level5_price = String(result.level5_price).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
								var level6_price = String(result.level6_price).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
								var level7_price = String(result.level7_price).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
								var level8_price = String(result.level8_price).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
								var level9_price = String(result.level9_price).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
								var goods_price = String(result.goods_price).replace(/\B(?=(\d{3})+(?!\d))/g, ",");

								$("#lv4_price").val(level4_price);
								$("#lv5_price").val(level5_price);
								$("#lv6_price").val(level6_price);
								$("#lv7_price").val(level7_price);
								$("#lv8_price").val(level8_price);
								$("#lv9_price").val(level9_price);
...
<div class="tbl_frm02">
	...
	<tr>
		<th scope="row">도매공급가격</th>
		<td>
			<input type="text" id="supply_price" name="supply_price" readonly value="<?php echo number_format($gs['supply_price']); ?>" class="frm_input w80" onkeyup="addComma(this);"> 원
			<span class="fc_197 marl5">사입처에서 공급받은 가격(수정불가)</span>
		</td>
	</tr>
	<tr>
		<th scope="row">일반소매회원판매가격</th>
		<td>
			<input type="text" id="lv6_price" name="lv6_price" value="<?php echo number_format($gs['lv6_price']); ?>" class="frm_input w80" onkeyup="calc(this);addComma(this);"> 원
			<span class="fc_197 marl5"></span>
		</td>
	</tr>
	<tr>
		<th scope="row">우수소매회원판매가격</th>
		<td>
			<input type="text" id="lv5_price" name="lv5_price" value="<?php echo number_format($gs['lv5_price']); ?>" class="frm_input w80" onkeyup="calc(this);addComma(this);"> 원
			<span class="fc_197 marl5"></span>
		</td>
	</tr>
	<tr>
		<th scope="row">특별소매회원판매가격</th>
		<td>
			<input type="text" id="lv4_price" name="lv4_price" value="<?php echo number_format($gs['lv4_price']); ?>" class="frm_input w80" onkeyup="calc(this);addComma(this);"> 원
			<span class="fc_197 marl5"></span>
		</td>
	</tr>
	<tr>
		<th scope="row">일반회원판매가격</th>
		<td>
			<input type="text" id="lv9_price" name="lv9_price" value="<?php echo number_format($gs['lv9_price']); ?>" class="frm_input w80" onkeyup="calc(this);addComma(this);"> 원
			<span class="fc_197 marl5"></span>
		</td>
	</tr>
	<tr>
		<th scope="row">우수회원판매가격</th>
		<td>
			<input type="text" id="lv8_price" name="lv8_price" value="<?php echo number_format($gs['lv8_price']); ?>" class="frm_input w80" onkeyup="calc(this);addComma(this);"> 원
			<span class="fc_197 marl5"></span>
		</td>
	</tr>
	<tr>
		<th scope="row">특별회원판매가격</th>
		<td>
			<input type="text" id="lv7_price" name="lv7_price" value="<?php echo number_format($gs['lv7_price']); ?>" class="frm_input w80" onkeyup="calc(this);addComma(this);"> 원
			<span class="fc_197 marl5"></span>
		</td>
	</tr>
	<tr>
		<th scope="row">소매판매가격</th>
		<td>
			<input type="text" name="goods_price" value="<?php echo number_format($gs['goods_price']); ?>" class="frm_input w80" onkeyup="addComma(this);"> 원
			<span class="fc_197 marl5">소매 실제 판매가 입력 (대표가격으로 사용)</span>
		</td>
	</tr>
	<tr>
		<th scope="row">포인트</th>
...
function fregform_submit(f) {
	...
	var supply_price = $("#supply_price").val().replace(/,/g, '');
	var lv7_price = $("#lv7_price").val().replace(/,/g, '');
	var lv8_price = $("#lv8_price").val().replace(/,/g, '');
	var lv9_price = $("#lv9_price").val().replace(/,/g, '');

	if(supply_price > lv7_price) {
        alert("공급가보다 일반회원판매가가 높아야 합니다.");
        return false;
    }
	if(supply_price > lv8_price) {
        alert("공급가보다 우수회원판매가가 높아야 합니다.");
        return false;
    }
	if(supply_price > lv9_price) {
        alert("공급가보다 특별회원판매가가 높아야 합니다.");
        return false;
    }
