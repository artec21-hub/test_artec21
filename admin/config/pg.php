	...
	<tr>
		...
		<td>
			<select name="de_pg_service" id="de_pg_service">
				...
				<?php echo option_selected("settle", $default['de_pg_service'], "세틀뱅크"); ?>
			</select>
...
<div class="pg_info_fld settle_info_fld">
	<h2>세틀뱅크 계약정보</h2>
	<div class="tbl_frm01">
		<table>
		<colgroup>
		...
		<tbody>
		<tr>
			<th scope="row"><label for="de_settle_mid">세틀뱅크 상점아이디</label></th>
			<td>
				<input type="text" name="de_settle_mid" value="<?php echo $default['de_settle_mid']; ?>" id="de_settle_mid" class="frm_input" size="10" maxlength="20">
				<a href="http://ecredit.uplus.co.kr/" target="_blank" class="btn_small blue">세틀뱅크 서비스신청하기</a>
			</td>
		</tr>
		<tr>
			<th scope="row"><label for="de_settle_license_key">세틀뱅크 라이센스키</label></th>
			<td>
				<input type="text" name="de_settle_license_key" value="<?php echo $default['de_settle_license_key']; ?>" id="de_settle_license_key" class="frm_input" size="40" maxlength="50">
				<em>테스트) ST1009281328226982205</em>
				<?php echo help('라이센스키는 세틀뱅크 상점관리자 <strong>[계약정보 &gt; 상점정보관리 &gt; 시스템연동정보]</strong>에서 확인하실 수 있습니다.'); ?>
			</td>
		</tr>
		<tr>
			<th scope="row"><label for="de_settle_aes256_key">세틀뱅크 AES256 암호화Key</label></th>
			<td>
				<input type="text" name="de_settle_aes256_key" value="<?php echo $default['de_settle_aes256_key']; ?>" id="de_settle_aes256_key" class="frm_input" size="40" maxlength="50">
				<em>테스트) pgSettle30y739r82jtd709yOfZ2yK5K</em>
				<?php echo help('세틀뱅크 AES256 암호화Key는 세틀뱅크 상점관리자 <strong>[계약정보 &gt; 상점정보관리 &gt; 시스템연동정보]</strong>에서 확인하실 수 있습니다.'); ?>
			</td>
		</tr>
		</tbody>
		</table>
...
if($default['de_iche_use'] || $default['de_vbank_use'] || $default['de_hp_use'] || $default['de_card_use']) {
    ...
    if($default['de_pg_service'] == 'lg') {
	 ...
	 // 세틀뱅크의 경우 log 디렉토리 체크
    if($default['de_pg_service'] == 'settle') {
        $log_path = TB_SETTLE_PATH.'/log';

        if(!is_dir($log_path)) {
            echo '<script>'.PHP_EOL;
            echo 'alert("'.str_replace(TB_PATH.'/', '', TB_SETTLE_PATH).'/settle 폴더 안에 log 폴더를 생성하신 후 쓰기권한을 부여해 주십시오.\n> mkdir log\n> chmod 707 log");'.PHP_EOL;
            echo '</script>'.PHP_EOL;
        } else {
            if(!is_writable($log_path)) {
                echo '<script>'.PHP_EOL;
