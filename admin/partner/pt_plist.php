...
$sql_search = " where grade between 3 and 6 ";
...
// 수수료합계
$row1 = sql_fetch(" select SUM(cash) as sum_cash {$sql_common} {$sql_search} ");
$stotal_cash = (int)$row1['sum_cash'];

$row2 = sql_fetch(" select SUM(point) as sum_point {$sql_common} {$sql_search} ");
$stotal_point = (int)$row2['sum_point'];
...
<div class="tbl_frm01">
	...
	<tr>
		...
		<td>
			<?php echo get_search_level('sst', $sst, 3, 6); ?>
		</td>
	</tr>
	</tbody>
	</table>
...
<div class="local_ov mart30">
	...
	<strong class="ov_a">총 예치금잔액 : <?php echo number_format($stotal_cash); ?>원 </strong>
	<strong class="ov_a">총 포인트잔액 : <?php echo number_format($stotal_point); ?>원 </strong>
</div>
...
<div class="tbl_head02">
	...
	<tr>
		...
		<th scope="col" colspan="3">예치금집계</th>
		<th scope="col" colspan="3">포인트집계</th>
	...
	<tr class="rows">
		...
		<th scope="col" class="th_bg"><?php echo subject_sort_link('pay', $q2); ?>현재잔액</a></th>
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$row_cash_minus = sql_fetch(" select SUM(cash_now) as sum_cash from shop_cash_history where id = '{$row[id]}' and plus_minus = '-' ");
		$total_cash_minus = (int)$row_cash_minus['sum_cash'];

		$row_cash_plus = sql_fetch(" select SUM(cash_now) as sum_cash from shop_cash_history where id = '{$row[id]}' and plus_minus = '+' ");
		$total_cash_plus = (int)$row_cash_plus['sum_cash'];

		$row_point_minus = sql_fetch(" select SUM(po_point) as sum_point from shop_point where mb_id = '{$row[id]}' and po_point < 0 ");
		$total_point_minus = (int)$row_point_minus['sum_point'];

		$row_point_plus = sql_fetch(" select SUM(po_point) as sum_point from shop_point where mb_id = '{$row[id]}' and po_point > 0 ");
		$total_point_plus = (int)$row_point_plus['sum_point'];
	...
	<tr class="<?php echo $bg; ?>">
		...
		<td class="tar"><?php echo number_format($row['cash']); ?></td>
		<td class="tar"><?php echo number_format($total_cash_plus); ?></td>
		<td class="tar"><?php echo number_format($total_cash_minus); ?></td>
		<td class="tar"><?php echo number_format($row['point']); ?></td>
		<td class="tar"><?php echo number_format($total_point_plus); ?></td>
		<td class="tar"><?php echo number_format($total_point_minus); ?></td>
