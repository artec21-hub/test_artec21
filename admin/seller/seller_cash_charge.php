...
$sql_common = " from shop_cash_charge a, shop_seller b ";
$sql_search = " where a.to_id = b.mb_id and a.gubun = '도매' ";
...
<h2> 예치금 충전 신청</h2>
<form name="fplist" id="fplist" method="post" action="./seller/seller_charge_update.php" onsubmit="return fplist_submit(this);" autocomplete="off">
...
<div class="tbl_frm01">
	...
	<tbody>	
	<tr>
		<th scope="row"><label for="pp_content">공급사</label></th>
		<td> <?php 
			$sql = " select company_name,mb_id from shop_seller";
			$result2 = sql_query($sql);
		...
		?>
			...
			<?php
				for($i=0; $row2=sql_fetch_array($result2); $i++) {
			?>				
				<option value="<?php echo $row2['mb_id'];?>"><?php echo $row2['company_name'];?></option>
			<?php } ?>
			</select>
		</td>
	</tr>
