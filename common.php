...
function king_post($url, $fields)
{
	...
	curl_setopt($ch, CURLOPT_POST, 1);                               // POST 전송 여부
...
/*
function king_post_stock($wmscode)
{
	...
	$headers = array( "content-type: application/json", "accept-encoding: gzip" );
	$url = 'http://caseking.co.kr/api/getstock.php';
	...
	if (function_exists('curl_init')) {
		...
		//curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);              // 요청결과를 문자열로 반환
		//curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 20);               // connection timeout : 10초
		//curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);                 // 원격 서버의 인증서가 유효한지 검사 여부
		...
		$array = json_decode($response, true); //이것을 php에서 사용하기 위해 array로 디코딩?
		if($array['status'] == "success"){
			$return_value = $array['stock'];
		}else{
			$return_value = "0";
		}
	}else{
