...
if($gubun == "사업자"){
	...
	$company_item       = trim($_POST['company_item']);
	$company_service       = trim($_POST['company_service']);
	$info_name       = trim($_POST['info_name']);
	$company_hompage       = trim($_POST['company_hompage']);
	$company_saupja_no       = isset($_POST['company_saupja_no'])           ? trim($_POST['company_saupja_no'])         : "";
	$company_email       = isset($_POST['company_email'])           ? trim($_POST['company_email'])         : "";
	$info_tel       = isset($_POST['info_tel'])           ? trim($_POST['info_tel'])         : "";
	$company_tel       = isset($_POST['company_tel'])           ? trim($_POST['company_tel'])         : "";
	$company_fax       = isset($_POST['company_fax'])           ? trim($_POST['company_fax'])         : "";
	$info_email       = isset($_POST['info_email'])           ? trim($_POST['info_email'])         : "";
	$company_zip       = isset($_POST['company_zip'])           ? trim($_POST['company_zip'])         : "";
	...
	$company_owner        = clean_xss_tags($company_owner);
	$company_item        = clean_xss_tags($company_item);
	$company_service        = clean_xss_tags($company_service);
	$info_name        = clean_xss_tags($info_name);
	$company_hompage        = clean_xss_tags($company_hompage);

	$company_email       = get_email_address($company_email);
	$info_email       = get_email_address($info_email);

	$company_saupja_no		= clean_xss_tags($company_saupja_no);
	$company_tel			= clean_xss_tags($company_tel);
	$info_tel			= clean_xss_tags($info_tel);
...
if($w == '' || $w == 'u') {
	...
	if($gubun == "사업자"){
		...
		$tmp_company_name = iconv('UTF-8', 'UTF-8//IGNORE', $company_name);
		if($tmp_company_name != $company_name) {
			alert('업체명을 올바르게 입력해 주십시오.');
		}

		if($msg = empty_mb_name($company_name))		alert($msg);
		if($msg = empty_mb_name($company_owner))		alert($msg);
		if($msg = empty_mb_name($company_item))		alert($msg);
		if($msg = empty_mb_name($company_service))		alert($msg);
		if($msg = empty_mb_name($info_name))		alert($msg);

		if($msg = empty_mb_email($company_email))	alert($msg);
		
		//if($msg = valid_mb_name($mb_name))	alert($msg);
		if($msg = valid_mb_email($company_email))	alert($msg);
		if($msg = prohibit_mb_email($company_email))	alert($msg);

		if($msg = empty_mb_email($info_email))	alert($msg);
		
		//if($msg = valid_mb_name($mb_name))	alert($msg);
		if($msg = valid_mb_email($info_email))	alert($msg);
		if($msg = prohibit_mb_email($info_email))	alert($msg);
...
if($w == '') {
	...
	$value['gubun']		= $gubun; //회원구분
	if($gubun == "사업자"){

		$value['name']			= $company_name; //이름
		$value['company_owner']			= $company_owner; //이름
		$value['company_item']			= $company_item; //이름
		$value['company_service']			= $company_service; //이름
		$value['company_hompage']			= $company_hompage; //이름
		$value['company_saupja_no']			= $company_saupja_no; //이름
		$value['email']			= $company_email; //이름
		$value['company_tel']			= $company_tel; //이름
		$value['company_fax']			= $company_fax; //이름
		$value['zip']			= $company_zip; //이름
		$value['addr1']			= $company_addr1; //이름
		$value['addr2']			= $company_addr2; //이름
		$value['addr3']			= $company_addr3; //이름
		$value['addr_jibeon']			= $company_addr_jibeon; //이름

		$value['info_name']			= $info_name; //이름
		$value['cellphone']			= $info_tel; //이름
		$value['info_email']			= $info_email; //이름
	...
	if($config['shop_id'] == NULL){
		$value['shop_id']		= 'admin'; //추천인아이디
		$value['grade']			= '9'; //레벨
	}else{
		$value['shop_id']		= $config['shop_id']; //추천인아이디
		$value['grade']			= '2'; //레벨
	}
	...
	if($gubun == "사업자"){
		mailer($config['company_name'], $super['email'], $company_email, $subject, $content, 1);
	}else{
	...
	if($gubun == "사업자"){
		mailer($company_name, $company_email, $super['email'], $subject, $content, 1);
	}else{
...
if($w == '' && $config['coupon_yes']) {
	...
	if($cp['cp_id'] && $cp['cp_use']) {
		...
		if($cp_used){
			if($gubun == "사업자"){
				insert_used_coupon($mb_id, $company_name, $cp);
			}else{
