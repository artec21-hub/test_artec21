...
<div class="page_wrap">
	<div class="lbt_box">
		<a href="./list.php?code=notice&admin=true&<?php echo $qstr1; ?>" class="btn_lsmall bx-white">목록</a>
	</div>
	<div class="rbt_box">
		<?php if(($member['index_no'] == $bo_writer) || is_admin()) { ?>
		<a href="./modify.php?code=notice&admin=true&<?php echo $qstr2; ?>" class="btn_lsmall bx-white">수정</a>
		<?php } ?>
		<?php if($member['index_no'] && $member['grade']<=$board['reply_priv'] && $board['usereply']=='Y') { ?>
		<a href="./reply.php?code=notice&admin=true&<?php echo $qstr2; ?>" class="btn_lsmall bx-white">답글</a>
		<?php } ?>
		<?php if(($member['index_no'] == $bo_writer) || is_admin()) { ?>
		<a href="./del.php?code=notice&admin=true&<?php echo $qstr2; ?>" class="btn_lsmall bx-white">삭제</a>
		<?php } ?>
		<?php if($member['grade'] <= $board['write_priv']){ ?>
		<a href="./write.php?code=notice&admin=true&boardid=<?php echo $boardid; ?>" class="btn_lsmall">글쓰기</a>
		<?php } ?>
	</div>
</div>
...
if(sql_num_rows($res)) {
?>
<div class="tbl_frm02 tbl_wrap2 marb10">
	<table>
	<tbody>
	<?php
	...
	?>
	<tr class="list1">
		<td>작성자 : <b><?php echo $row['writer_s']; ?></b> (<?php echo $bo_wdate; ?>) <?php echo "<a href=\"./tail_del.php?admin=true&code=notice&tailindex={$row['index_no']}&{$qstr2}\" class=\"btn_ssmall bx-white\">삭제</a>"; ?></td>
	</tr>
	<tr>
...
<?php
include_once(TB_BBS_PATH."/skin/admin/read_list.php");
?>