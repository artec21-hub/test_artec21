...
else // 장바구니에 담기
{
	...
	$select_array = explode(',', $select_rows);
	...
	for($i=0; $i<$count; $i++) {
		...
		//echo $pt_id;
		$gs['somae_supply_price'] = get_somae_price($gs_id,$pt_id);
        ...
        for($k=0; $k<$opt_count; $k++) {
            ...
            $king_pcode = $_POST['king_pcode'][$gs_id][$k];
            ...
            if(!$io_id)
				...
				$it_stock_qty = get_option_stock_qty($king_pcode);
//                $it_stock_qty = get_option_stock_qty($gs_id, $io_id, $io_type);
		...
		for($k=0; $k<$opt_count; $k++) {
			...
			$sql = " insert into shop_cart
						( ca_id, mb_id, gs_id, ct_direct, ct_time, ct_price, ct_supply_price, ct_somae_supply_price, ct_qty, ct_point, king_pcode, io_id, io_type, io_supply_price, io_price, ct_option, ct_send_cost, od_no, ct_ip )
					VALUES ";
			$sql.= "( '$ca_id', '{$member['id']}', '{$gs['index_no']}', '$set_cart_id', '".TB_TIME_YMDHIS."', '{$gs['goods_price']}', '{$gs['supply_price']}', '{$gs['somae_supply_price']}', '$ct_qty', '{$gs['gpoint']}', '$king_pcode', '$io_id', '$io_type', '$io_supply_price', '$io_price', '$io_value', '$ct_send_cost', '$od_no', '{$_SERVER['REMOTE_ADDR']}' )";
