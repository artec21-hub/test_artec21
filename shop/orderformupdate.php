...
for($i=0; $row=sql_fetch_array($result); $i++) {
 ...
 //       $it_stock_qty = (int)get_option_stock_qty($row['gs_id'], $row['io_id'], $row['io_type']);
       $it_stock_qty = (int)get_option_stock_qty($row['king_pcode']);
    } else {
...
if($_POST['paymethod'] == '무통장'){
	$dan = 1; // 주문접수 단계로 적용
}elseif($_POST['paymethod'] == '예치금'){
	$dan = 2; // 결제완료 단계로 적용
	cash_charge($member['id'],'admin',(int)$_POST['use_cash'],'가맹점 물품구매로 예치금 차감','가맹점','minus');
}
...
set_session('tot_price', (int)str_replace(',','',$_POST['tot_price']));
set_session('use_point', (int)$_POST['use_point']);
set_session('use_cash', (int)$_POST['use_cash']);
...
$use_cash = (int)$_POST['use_cash']; // 예치금결제
...
for($i=0; $i<count($gs_id); $i++) {
	...
	$sql = "insert into shop_order
			   set od_id				= '{$od_id}'
				 ...
				 , somae_supply_price	= '{$somae_supply_price[$i]}'
...
if(in_array($_POST['paymethod'],array('무통장','포인트','예치금'))) {
...
if(in_array($_POST['paymethod'],array('무통장','포인트','예치금'))) {
	...
	else if($default['de_pg_service'] == 'settle')
		goto_url(TB_SHOP_URL.'/ordersettle.php?od_id='.$od_id);
}
?>
